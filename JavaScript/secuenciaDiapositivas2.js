...
function generarPantalla() {
  ...
  for(var i = 0; i < juegoDiap.length; i ++) {
    ...
    /*menuStr += '<img class="thumbnail" src="chicas/' + juegoDiap[i].chica + '.jpg"' + 'onMouseOver="clearTimeout(temporiz); clearInterval(cuenta); selectDIV(' + i + '); diaposelec = i;'
      + 'onMouseOut="temporiz = setTimeout(function(){imgAuto()}, 1000);">';*/
    menuStr += '<li><a ' + 'href="' + juegoDiap[i].chica + '.html" ' + 'onMouseOver="clearTimeout(temporiz); clearInterval(cuenta); selectDIV(' + i + '); diaposelec = i;' + 
    'return true;" ' +
    'onClick="return false;" ' + 'onMouseOut="temporiz = setTimeout(function(){imgAuto()}, 1000);">' + '<img src="chicas/' + juegoDiap[i].chica + '.jpg" ' + 
    'style="heigth:50px; width:100px;">' + '<br />' + juegoDiap[i].chica + '</a></li><br />';
  }
  menuStr += '</ul><br />';
  document.write(
    '<div style= "position: absolute; top: 20%; width: 40%;">' + menuStr + '<div>'
  );
  // Creamos el layer del menú
  //genLayer('menu_guia', sWidPos - 204, 43, 200, 200, mostrarDIV, '<div id="menu_guia">' + '<b>Nuestras chicas</b><br />' + menuStr + '</div>');
  imgAuto();
}
//================================================================================================================
function imgAuto() {
